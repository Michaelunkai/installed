...
from PyQt5.QtWidgets import (
    ...
    QCheckBox, QTextEdit, QComboBox, QMenu, QMainWindow
)
...
def pull_docker_image(tag, timeout=600):
    ...
    try:
        ...
        print(f"\nTimeout pulling image for {tag}. The operation took longer than {timeout} seconds.")
        process.kill()
...
def run_docker_command(tag, destination_path, progress_callback=None):
    ...
    try:
        ...
        docker_cmd = (
            f"powershell -Command \"drun {tag} michadockermisha/backup:{tag} sh -c 'apk add rsync pigz && mkdir -p /games/{tag} && rsync -aP --compress-level=1 --compress --numeric-ids --inplace --delete-during --info=progress2 --no-i-r /home/ /games/{tag}'\""
        )
        ...
        print(f"Error in run_docker_command: {e}")
        return None, None
...
class DockerApp(QWidget):
    ...
    def initialize_components(self):
        ...
        try:
            ...
            QTimer.singleShot(0, self._initialize_background)
    ...
    def create_tag_buttons(self):
        ...
        thread_pool = QThreadPool()
        thread_pool.setMaxThreadCount(min(8, len(self.all_tags)))
    ...
    def run_docker_in_thread(self, command, tag_name="", target_path=""):
        """Run a Docker command in a worker thread to avoid blocking the GUI"""
        worker = DockerProgressWorker(command, tag_name, target_path)
        
        # Connect signals
        worker.signals.started.connect(self.on_docker_started)
        worker.signals.progress.connect(self.on_docker_progress)
